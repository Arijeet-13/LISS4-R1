...
class CausalOutputWithMask(CausalLMOutputWithPast):
    ...
    loss_channel: Optional[torch.FloatTensor] = None
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, query, key_value):
        ...
        query = self.norm1(query + attn)

        out = self.ffn(query)

        out = self.norm2(query + out)
...
class AttentionLoss(nn.Module):
    ...
    def forward(self, model_attention_logits: torch.Tensor, gt_mask: torch.Tensor) -> torch.Tensor:
        ...
        model_attention_logits = model_attention_logits.float()  # compute in fp32 for numerical stability
        
        # Initialize loss
        loss = torch.tensor(0.0, device=device)  # Make sure the tensor is on the correct device
        epsilon = 1e-4  # To avoid log(0) #Better for fp16
...
class SegEarthR2(MiphaPhiForCausalLM):
    ...
    dim=256,
        ...
        self.language_mamba_bwd = None
        self.language_gate = None'''
        
        
        '''self.language_norm = nn.LayerNorm(config.hidden_size)

        self.language_mamba_fwd = Mamba(
    ...
    expand=2,
        ...
        self.language_gate = nn.Parameter(torch.tensor(0.5))'''
    ...
    def initial_mask_module(self, pretrained_path=None, model_args=None):
        ...
        self.channel_loss = ChannelDiscriminabilityLoss()
    ...
    def forward(
        ...
        '''hidden = self.language_norm(hidden_states)

        fwd = self.language_mamba_fwd(hidden)

        bwd = self.language_mamba_bwd(
    hidden.flip(1)
        ).flip(1)

        hidden_states = hidden_states + self.language_gate * (fwd + bwd)'''
        
        logits = self.lm_head(hidden_states)
        if SEG_token_embedding_indices is not None:
            attentions = [attention_item.sum(dim=1) for attention_item in outputs.attentions]
            # attentions = [self._attn_weights_cache[i].sum(dim=1) for i in sorted(self._attn_weights_cache.keys())]
            # self._attn_weights_cache.clear()
    ...
    multi_scale_features
            ...
            loss = llm_loss + mask_loss + 0.01 * loss_attention + 0.005 * loss_channel
    ...
    multi_scale_features
        ...
        #Here are changes
        # mask_outputs = self.predictor(multi_scale_features, mask_features, None, None, SEG_embedding) 
        mask_pred_results = mask_outputs["pred_masks"]   # raw predictor output, no resizing yet

        images = ImageList.from_tensors(images, self.size_divisibility)  # still needed if used elsewhere; otherwise can drop
        ...
        for _seg_info, mask_pred_result in zip(seg_info, mask_pred_results):
            ...
            if gt_mask is not None:
                ...
                gt_mask_t = torch.as_tensor(gt_mask, dtype=torch.float32, device=mask_pred_result.device)
                gt_h, gt_w = gt_mask_t.shape[-2], gt_mask_t.shape[-1]

                # Single resize: predictor's raw output resolution -> GT's native size directly.
                pred_native = F.interpolate(
                    mask_pred_result.unsqueeze(0).float(),
                    size=(gt_h, gt_w),
                    mode="bilinear",
                    align_corners=False,
                ).squeeze(0)

                pred_bin = (pred_native > 0)
                gt_bin = (gt_mask_t > 0.5)
            else:
                pred_bin = (mask_pred_result > 0)
                gt_bin = None

            instance_r = {
                'pred': (pred_bin.cpu().numpy() * 255).astype(np.uint8),
                'gt': (gt_bin.cpu().numpy() * 255).astype(np.uint8) if gt_bin is not None else None,
    ...
    def inference(
        ...
        '''hidden = self.language_norm(last_hidden_states)
    ...
    hidden.flip(1)
        ...
        last_hidden_states = last_hidden_states + self.language_gate * (fwd + bwd)'''
