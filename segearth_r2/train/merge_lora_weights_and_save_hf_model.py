...
def load_pretrained_model(model_path, model_args, mask_config='/mask_config/maskformer2_swin_base_384_bs16_50ep.yaml', load_8bit=False, load_4bit=False, device_map="auto", device="cuda"):
    kwargs = {"low_cpu_mem_usage": False}  #Added to remove the error
    ...
    model = model.to('cpu')  #Added to remove the error
    ...
    model.resize_token_embeddings(max(len(tokenizer), model.config.vocab_size)) #Changed due to errors
