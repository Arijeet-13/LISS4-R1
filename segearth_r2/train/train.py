...
def make_unify_datamodule(clip_image_processor, tokenizer, data_args, training_args):
    ...
    datasets = []
    if data_args.dataset_type == 'EarthReason': #Added EarthReason Setup
        ...
        datasets += [EarthReasonTrainDataset] * data_ratio[0]
    elif data_args.dataset_type == 'LISS4Reason': #Added LISS4Reason Setup
            LISS4ReasonTrainDataset = LISS4ReasonDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args)
            datasets += [LISS4ReasonTrainDataset] * data_ratio[0]
    elif data_args.dataset_type == 'RefSegRS': #Added RefSegRS Setup
            RefSegRSTrainDataset = RefSegRSDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args)
            datasets += [RefSegRSTrainDataset] * data_ratio[0]
            ...
            RISBenchTrainDataset = RISBenchDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args)
            datasets += [RISBenchTrainDataset] * data_ratio[0]
    else:
        if data_ratio[0] != 0:
...
def train():
    ...
    # model._attn_weights_cache = {}
    
    # def make_attn_hook(layer_idx, cache):
    #     def hook(module, input, output):
    #         if isinstance(output, tuple) and len(output) > 1 and output[1] is not None:
    #             cache[layer_idx] = output[1]
    #     return hook
    
    # for idx, layer in enumerate(model.model.layers):
    #     layer.self_attn.register_forward_hook(make_attn_hook(idx, model._attn_weights_cache))
